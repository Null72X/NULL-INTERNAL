    ...
    internal class ESP : ClickableTransparentOverlay.Overlay
    {
        ...
        protected override unsafe void Render()
        {
            if (!_fontInitialized)
            {
                try
                {
                    ReplaceFont(@"C:\Windows\Fonts\bahnschrift.ttf", 14, FontGlyphRangeType.English);
                }
                catch
                {
                    try
                    {
                        ReplaceFont(@"C:\Windows\Fonts\segoeuib.ttf", 14, FontGlyphRangeType.English);
                    }
                    catch
                    {
                        try
            ...
            if (isInMatch)
            {
                if (Config.Text)
                {
                    ...
                    // LED circle dimensions
                    float dotRadius = 4f;
                    float dotSpacing = 8f;
                    float rangeRowWidth = (dotRadius * 2f) + dotSpacing + rangeSize.X;

                    // Container measurements
                    float maxWidth = Math.Max(titleSize.X, Math.Max(subSize.X, rangeRowWidth));
                    float padX = 24f;
                    float padY = 14f;

                    float cardWidth = maxWidth + padX * 2f;
                    float totalTextHeight = titleSize.Y + subSize.Y + rangeSize.Y;
                    float spacingY = 16f; // total vertical spacing inside card
                    float cardHeight = totalTextHeight + spacingY + padY * 2f;

                    float centerX = Core.Width / 2f;
                    float baseY = 65f;

                    Vector2 cardMin = new Vector2(centerX - cardWidth / 2f, baseY);
                    Vector2 cardMax = new Vector2(centerX + cardWidth / 2f, baseY + cardHeight);

                    // 1. Draw premium translucent card background (glassmorphism overlay style)
                    drawList.AddRectFilled(cardMin, cardMax, ColorToUint32(Color.FromArgb(170, 12, 12, 14)), 8f, ImDrawFlags.RoundCornersAll);
                    // 2. Draw modern high-tech thin border
                    drawList.AddRect(cardMin, cardMax, ColorToUint32(Color.FromArgb(45, 255, 255, 255)), 8f, ImDrawFlags.RoundCornersAll, 1f);
                    // 3. Glowing primary accent bar at the top of the HUD card
                    drawList.AddLine(new Vector2(cardMin.X + 10f, cardMin.Y), new Vector2(cardMax.X - 10f, cardMin.Y), accent, 2.5f);

                    float currentY = cardMin.Y + padY + 2f;

                    // Draw Title (Centered)
                    Vector2 titlePos = new Vector2(centerX - titleSize.X / 2f, currentY);
                    drawList.AddText(titlePos + new Vector2(1, 1), ColorToUint32(Color.Black), title); // Shadow
                    drawList.AddText(titlePos, accent, title);
                    currentY += titleSize.Y + 5f;

                    // Separator line
                    drawList.AddLine(new Vector2(centerX - 40f, currentY), new Vector2(centerX + 40f, currentY), ColorToUint32(Color.FromArgb(30, 255, 255, 255)), 1f);
                    currentY += 6f;

                    // Draw Subtitle (Centered)
                    Vector2 subPos = new Vector2(centerX - subSize.X / 2f, currentY);
                    drawList.AddText(subPos + new Vector2(1, 1), ColorToUint32(Color.Black), subtitle); // Shadow
                    drawList.AddText(subPos, white, subtitle);
                    currentY += subSize.Y + 8f;

                    // Draw Status LED & Enemies in Range Row (Centered together)
                    float rowStartX = centerX - rangeRowWidth / 2f;
                    Vector2 dotCenter = new Vector2(rowStartX + dotRadius, currentY + rangeSize.Y / 2f);
                    Vector2 rangePos = new Vector2(rowStartX + (dotRadius * 2f) + dotSpacing, currentY);

                    uint ledColor;
                    if (enemyCount > 0)
                    {
                        float pulse = (float)(Math.Sin(ImGui.GetTime() * 6f) * 0.5f + 0.5f);
                        ledColor = ColorToUint32(Color.FromArgb(255, (int)(225 + pulse * 30), (int)(45 - pulse * 15), (int)(45 - pulse * 15)));
                        // Glowing outer ring for danger
                        drawList.AddCircle(dotCenter, dotRadius + 1f + pulse * 2.5f, ColorToUint32(Color.FromArgb((byte)(90 - pulse * 70), 255, 45, 45)), 12, 1.5f);
                    }
                    else
                    {
                        ledColor = ColorToUint32(Color.FromArgb(255, 50, 205, 50)); // Green when safe
                    }

                    // Draw LED Dot
                    drawList.AddCircleFilled(dotCenter, dotRadius, ledColor, 12);
                    drawList.AddCircle(dotCenter, dotRadius, ColorToUint32(Color.Black), 12, 1f); // black ring

                    // Draw Text
                    drawList.AddText(rangePos + new Vector2(1, 1), ColorToUint32(Color.Black), rangeText); // Shadow
                    drawList.AddText(rangePos, rangeColor, rangeText);
                }
